	...
	<div class="space-y-6 animate-fade-in">
			...
			<div v-if="mode === 'batch'" class="space-y-3">
				<div class="flex gap-3">
					<div class="flex-1">
						...
						<select v-model="batchKelas" @change="loadSiswaBatch" class="input">
				...
				<div v-if="batchSiswa.length" class="border rounded-xl overflow-hidden">
				...
				<div class="flex items-start gap-4">
					...
					<div>
						...
						<p v-if="scanResult.data" class="text-sm text-slate-600 mt-1">
							...
							{{ scanResult.data.kelasSnapshot }} | {{ scanResult.data.status }}
		...
		<div class="card">
			...
			<div class="grid grid-cols-4 gap-2 mb-4">
				...
				>
					<div :class="['text-xl font-bold', statColor[key]]">{{ count }}</div>
				...
				<table class="table">
					...
					<tbody>
						<tr v-for="(a, i) in absensiList" :key="a.id" class="group">
							...
							<td>
								<span :class="statusAbsenBadge(a.status)">{{ a.status }}</span>
							</td>
